...
function applyFiltersAndRender() {
    ...
    if (searchInput && searchInput.value) {
        ...
        sortedAndFiltered = sortedAndFiltered.filter(p => p.name.toLowerCase().includes(keyword));
    }
...
function renderData(data) {
    ...
    data.forEach((product) => {
        // Fallback logic giống bên User page
...
window.editProduct = (id) => {
    ...
    getProductAPI(id).then((res) => {
        ...
        // Mở modal
        const btnOpen = document.querySelector('[data-modal-target="crud-modal"]');
...
function updateProduct() {
    ...
    putAPI(editId, product).then((res) => {
        renderListProduct();
        // Click the specific close button in the modal to avoid triggering the open event listener
        const btnClose = document.querySelector('#crud-modal button[data-modal-toggle="crud-modal"]');
...
if (btnOpenAdd) {
    btnOpenAdd.addEventListener("click", () => {
        // Always reset form when explicitly clicking the "Thêm Sản Phẩm Mới" button, unless it's triggered programmatically for editing
